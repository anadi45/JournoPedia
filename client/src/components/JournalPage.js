    ...
    <div className="journal-page-div">
      ...
      <div className="auth-inner">
        <form>
          ...
          <div className="mb-3">
      <button className="btn btn-primary" onClick={handleClick}>Add More Authors</button>
      ...
      {Array.from(Array(counter)).map((c, index) => {
        return (
          ...
          <div className="mb-3">
              ...
              <input type="text" name="reviewer1" className="form-control" autoComplete="off" onChange={(e)=>{
                setReviewer([...new Set([...reviewer,e.target.value])]);
              }}/>
              <label>Reviewer 2</label>
              <input type="text" name="reviewer2" className="form-control" autoComplete="off" onChange={(e)=>{
                // setReviewer([...reviewer,e.target.value])
                setReviewer([...new Set([...reviewer,e.target.value])]);
              }}/>
              <label>Reviewer 3</label>
              <input type="text" name="reviewer3" className="form-control" autoComplete="off" onChange={(e)=>{
                // setReviewer([...reviewer,e.target.value]);
                setReviewer([...new Set([...reviewer,e.target.value])]);
              }}/>
              <label>Reviewer 4</label>
              <input type="text" name="reviewer4" className="form-control" autoComplete="off" onChange={(e)=>{
                // setReviewer([...reviewer,e.target.value]);
                setReviewer([...new Set([...reviewer,e.target.value])]);
              }}/>
          </div>
